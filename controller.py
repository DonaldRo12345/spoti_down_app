...
from settings.settings import EXCEL_PATH
...
class Controller:
    ...
    def __init__(self, view, sp_client: SpotifyCustomer):
        ...
        self.excel_handler = ExcelFileHandler(file_dir=EXCEL_PATH)
            ...
            for song in song_title:
                try:
                    ...
                    if self.view.textbox_csv:
                        ...
                        count += 1
                        text = f"{count}/{len(song_title)} : {progress*100:.2f}%"
                        self.view.text_generer_percent.configure(text=text)
        ...
        try:
            ...
            for i, songs in enumerate(self.search_songs):
                ...
                if i + 1 == len(self.search_songs):
                    self.is_song_loading = False
            if not self.is_song_loading and len(self.search_songs) > 0:
                showinfo(
                ...
                if len(self.items_selected) > 0:
                    if playlist_name:
                        ...
                        for song in self.items_selected:
                            ...
                            try:
                                self.view.text_transferer_percent.configure(text=text)
                            except Exception as error:
                                continue
    ...
    def download_song(self, url_crypte) -> None:
        ...
        print("decrypt_word:: ", decrypt_word)
        ...
        #     pool.apply_async(self.download_one_song, args=(decrypt_word,))
        #     print()
        # import threading
        # download = Downloader(
        #     sp_client=self.sp_client,
        ...
        #     logger=logger,
        # )
        thread = threading.Thread(target=self.download_one_song, args=(decrypt_word,))
        thread.start()
