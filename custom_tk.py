...
from async_controller import AsyncController, BaseController
from controller import Controller
from settings import settings
from settings.settings import BASE_IMAGE_PATH
...
def get_api_configuration():
    ...
    conf = APIConfig
    conf.SPOTIFY_CLIENT_ID = os.environ.get("SPOTIFY_CLIENT_ID")
    conf.USER_ID = os.environ.get("USER_ID")
    conf.SPOTIPY_REDIRECT_URI = os.environ.get("SPOTIPY_REDIRECT_URI")
    conf.SPOTIFY_CLIENT_SECRET_KEY = os.environ.get("SPOTIFY_CLIENT_SECRET_KEY")
...
class App(customtkinter.CTk):
    ...
    logo = customtkinter.CTkImage(Image.open(BASE_IMAGE_PATH / "mnlv-audio.png"), size=(200, 145))
    logo_welcome = customtkinter.CTkImage(
        Image.open(BASE_IMAGE_PATH / "ekila-downaudio.jpg"), size=(859, 145),
        #  Image.open(os.path.join(image_base_path, "login.png")), size=(859, 145)
    )
    pub_image = customtkinter.CTkImage(Image.open(BASE_IMAGE_PATH / "phone.jpg"), size=(250, 114))

    # button logo's
    search_image = customtkinter.CTkImage(
        Image.open(BASE_IMAGE_PATH / "icon/rechercher.png"),
        size=(GLIPH_ICON_WIDTH, GLIPH_ICON_HEIGHT),
    )
    transfert_image = customtkinter.CTkImage(
        Image.open(BASE_IMAGE_PATH / "icon/transférer.png"),
        size=(GLIPH_ICON_WIDTH, GLIPH_ICON_HEIGHT),
    )
    download_image = customtkinter.CTkImage(
        Image.open(BASE_IMAGE_PATH / "icon/télécharger.png"),
        size=(GLIPH_ICON_WIDTH, GLIPH_ICON_HEIGHT),
    )
    convert_image = customtkinter.CTkImage(
        Image.open(BASE_IMAGE_PATH / "icon/convertir.png"),
        size=(GLIPH_ICON_WIDTH, GLIPH_ICON_HEIGHT),
    )
    quit_image = customtkinter.CTkImage(
        Image.open(BASE_IMAGE_PATH / "icon/quitter.png"),
        size=(GLIPH_ICON_WIDTH, GLIPH_ICON_HEIGHT),
    )
    ...
    def __init__(self, user_login):
        ...
        self.text_tranfert = None

        self.text_generer_percent = None
    ...
    def set_controller(self, controller: BaseController):
        self.async_controller = controller
    ...
    def sidebar(self):
        ...
        self.appearance_mode_optionemenu = customtkinter.CTkOptionMenu(
            self.sidebar_frame,
            values=["Mode sombre","Mode clair", "Mode système"],
    ...
    def menu_bar(self):
        ...
        menu_bar = Menu(self, font=("Segoe UI", 18, "bold"))
        self.config(menu=menu_bar)

        menu_file = Menu(menu_bar, tearoff=0, font=("Segoe UI", 16))
        menu_help = Menu(menu_bar, tearoff=0, font=("Segoe UI", 16))
    ...
    def extrat_csv_son_panel(self):
        ...
        self.text_generer_percent = customtkinter.CTkLabel(
            self.dashboard_frame,
            text="",
            font=customtkinter.CTkFont(size=15, weight="bold"),
        )
        self.text_generer_percent.grid(column=1, row=3, sticky="ne", padx=150)
    ...
    def transfert_son_panel(self):
        ...
        self.transfert_frame.grid(
            row=2, column=1, rowspan=4, columnspan=1, sticky="nsew"
        )
        ...
        self.text_transferer_percent = customtkinter.CTkLabel(
            self.button_frame,
            text="",
            font=customtkinter.CTkFont(size=15, weight="bold"),
            
        )
        self.text_transferer_percent.grid(column=3, row=0, padx=10)
    ...
    def download_son_panel(self):
        ...
        self.download_frame_progress = customtkinter.CTkFrame(
            self.download_frame,
            width=800, height=250
        )
        self.download_frame_progress.pack_propagate(False)
        self.download_frame_progress.grid(row=2, column=1, pady=5, sticky="nw")

        self.canvas = tk.Canvas(self.download_frame_progress, width=780, height=230, bg="#1e1e1e")
        self.canvas.pack(side="left", fill="both", expand=True)

        self.scrollbar = Scrollbar(self.download_frame_progress, orient="vertical", command=self.canvas.yview)
        self.scrollbar.pack(side="right", fill="y")

        self.canvas.configure(yscrollcommand=self.scrollbar.set)

        # Ajouter une frame interne dans le canvas pour les widgets
        self.inner_frame = customtkinter.CTkFrame(self.canvas, width=780, height=230, fg_color="#2e2e2e")
        self.canvas.create_window((3, 3), window=self.inner_frame, anchor="nw")

        self.async_controller.show_song_widgets()


        self.inner_frame.bind("<Enter>", self._bind_scroll)
        self.inner_frame.bind("<Leave>", self._unbind_scroll)

        # Mettre à jour la taille de la fenêtre interne dynamiquement
        ...
        self.download_sons_button = customtkinter.CTkButton(
            ...
            command=lambda: self.async_controller.start_download(),
            # command=lambda: self.async_controller.main_get_event_loop(),
        )
        ...
        self.text_percent = tk.Label(
            self.download_frame,
            fg="white", # Couleur du texte
            bg="#2e2e2e",  # Fond blanc
            #bold
            font=("", 11),
        )
        self.text_percent.grid(row=3, column=1, sticky="ne", pady=7, padx=120)
        self.download_progressbar.set(0)

    def _bind_scroll(self, event):
        """Lier le défilement de la souris à la barre de défilement."""
    ...
    def _unbind_scroll(self, event):
        """Délier le défilement de la souris."""
